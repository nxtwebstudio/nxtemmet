...
define(function(require, exports, module) {
	...
	function isLastChild(item) {
	    var itemStart = item.start;
	    var currentIndex = item.index();
	    var totalChildren = item.parent.children.length;
	...
	function processTag(item, profile) {
	    ...
	    // @hack - force new lines for empty tags <img />
	    // isUnary = false;
	    // profile.tag_nl_leaf = 1;
		...
		if (profile.tag_nl !== false) {
	      ...
	      if (!item.isTextNode()) {
	        ...
	        // check if it is not the last child
	        if( !isLastChild(item) ) {
	          item.end += nl;
	        }
